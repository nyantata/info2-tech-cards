(function(){
  ...
  function practiceHtml(id){
    ...
    if(id==='p2')return `<div class="space-y-5"><p class="font-bold leading-8">次の2モデルを、データを5つに分けて評価部分を入れ替える「5分割交差検証」で比べます。平均RMSEが小さい方を選んでください。</p><button id="run-model-compare" class="btn">公式データで比較を実行</button><div id="model-compare-result" class="grid md:grid-cols-2 gap-4"></div><div class="space-y-3"><label class="quiz-option"><input type="radio" name="p2" value="A" ${state.answers.p2==='A'?'checked':''}><span>モデルA：所得・リサイクル率・高齢化率</span></label><label class="quiz-option"><input type="radio" name="p2" value="B" ${state.answers.p2==='B'?'checked':''}><span>モデルB：モデルA＋人口密度</span></label></div></div>`;
...
y_pred = <select id="code3"><option value="">選択</option><option value="test">model.predict(X_test)</option><option value="train">model.predict(X_train)</option></select></pre></div>`;
    return `<div class="space-y-5"><p class="font-bold leading-8">公式データを5つに分けて繰り返し評価した結果、モデルAの平均RMSEはモデルBより小さくなりました。最も適切な解釈を選んでください。</p><div class="space-y-3"><label class="quiz-option"><input type="radio" name="p4" value="0" ${state.answers.p4===0?'checked':''}><span>説明変数を増やしたモデルBの方が必ず正しい</span></label><label class="quiz-option"><input type="radio" name="p4" value="1" ${state.answers.p4===1?'checked':''}><span>今回の繰り返し評価ではモデルBの予測誤差が大きい。人口密度を加えるかどうか見直す必要がある</span></label><label class="quiz-option"><input type="radio" name="p4" value="2" ${state.answers.p4===2?'checked':''}><span>R²が負なのでデータは利用できない</span></label></div></div>`
  }
  function render(){
  ...
  function score(){let knowledge=0,practical=0,details=[];for(const q of questions.slice(0,4)){const ok=state.answers[q.id]===q.ans;if(ok)knowledge+=10;details.push({name:q.title,ok,exp:q.exp})}
    ...
    const p2ok=state.answers.p2==='A';if(p2ok)practical+=15;details.push({name:'実技2',ok:p2ok,exp:'5回の評価で求めた平均RMSEはモデルAの方が小さく、モデルBよりモデル作成に使っていないデータに対する予測誤差が小さい結果です。'});
